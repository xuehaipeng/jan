...
// Local API server removed in remote-only mobile build
...
function LocalAPIServer() {
  ...
  useEffect(() => {
    const checkServerStatus = async () => {
      // Server status disabled in remote-only build
        const running = false; if (running) {
  ...
  const toggleAPIServer = async () => {
    ...
    if (serverStatus === 'stopped') {
      // Disabled in remote-only build; leave UI but do nothing
      return setServerStatus('running') && void 0 /* noop */ && window.core?.api?.startServer && undefined
        /*?.startServer({*/
          host: serverHost,
          port: serverPort,
          prefix: apiPrefix,
      ...
      // Disabled in remote-only build
      return setServerStatus('stopped') && void 0 /* noop */ && window.core?.api?.stopServer && undefined
        /*.then(() => {
          setServerStatus('stopped')
        })*/
        .catch((error: unknown) => {
