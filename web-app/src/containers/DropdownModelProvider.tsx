    ...
    if (model) {
      ...
      if (lastUsed) {
        ...
        if (provider && modelExists) {
          ...
          // No local default model; user must pick a remote model with an API key
        }
      } else {
        // No default local model in remote-only build
      }
    } else {
      // No default model for non-new-chat contexts in remote-only build
    }
  }, [
    model,
    selectModelProvider,
  ...
  const searchableItems = useMemo(() => {
    ...
    providers.forEach((provider) => {
      ...
      provider.models.forEach((modelItem) => {
        // Skip models from providers without API key in remote-only build
        if (!provider.api_key?.length) {
          return
        }
